...
const Player = () => {
    ...
    if (!activeSong) return <p style={{fontSize:"larger", display:"flex",justifyContent:"center"}}>Please select a song</p>

    const audioFileAddress = `${window.location.origin}/${songDetails.fileAddress}`;

    return (
        <div > 
            <audio controls ref={audioRef} key={activeSong.id} className="audio-player">
